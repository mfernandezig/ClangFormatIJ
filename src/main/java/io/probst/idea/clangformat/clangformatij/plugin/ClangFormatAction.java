//#***************************************************************************
//# freenet Technologie und Prozesse Source File: ClangFormatAction.java
...
public class ClangFormatAction extends AnAction
{
		...
		try
		{
			...
			ranges = ranges.stream().map(r ->
					new TextRange(toByteOffset(document.getText(), r.getStartOffset()),
						toByteOffset(document.getText(), r.getEndOffset()))).collect(Collectors.toList());
	 ...
	 * DOCME
	 *
	 * @param replacements DOCME
	 * @param project      DOCME
	 * @param document     DOCME
	 * @param file         DOCME
	 * @param caret        DOCME
	 */
	void applyChange(Replacements replacements, Project project, Document document, PsiFile file, Caret caret)
	{
			...
			WriteCommandAction.runWriteCommandAction(project, "Format Code", null,
				() ->
				{
					...
					for (Replacement r : replacements.getReplacements())
					{
						...
						document.replaceString(toCharOffset(bytes, actualStart), toCharOffset(bytes, actualEnd), r.getValue());
					}
	 ...
	 *
	 * @param project  DOCME
	 * @param errorMsg DOCME
	 */
	public static void showError(Project project, String errorMsg)
	{
		Notification notification = new Notification("ClangFormat", "Formatting Failed", errorMsg, NotificationType.ERROR);
	 ...
	 * @return DOCME
	 */
	protected Collection<TextRange> getFormatRanges(Project project, Document document, Editor editor, VirtualFile file)
	{
